...
// Route để lấy danh sách nhà xuất bản và mã để vào dropdown
  ...
  router.get('/', (req, res) => {
    ...
    nxb.TenNXB,
    s.MaTacGia,
    tg.TenTacGia,  -- Thêm tên tác giả từ bảng tacgia
    s.TheLoai,
    s.NamXuatBan,
    s.SoLuong,
    s.Anh
...
JOIN
  ...
  router.post('/themsach', (req, res) => {
    const { TenSach, MaTacGia, TheLoai, MaNXB, NamXuatBan, SoLuong } = req.body;
    ...
    const query = `
      INSERT INTO sach (TenSach, MaTacGia, TheLoai, MaNXB, NamXuatBan, SoLuong)
      VALUES (?, ?, ?, ?, ?, ?)
    `;
  
    db.query(query, [TenSach, MaTacGia, TheLoai, MaNXB, NamXuatBan, SoLuong], (err, result) => {
      if (err) {
  ...
  router.put('/api/sach/:id', (req, res) => {
    const { TenSach, TheLoai, MaTacGia, NamXuatBan, SoLuong, MaNXB } = req.body;
    const maSach = req.params.id;
  
    const updateSachSQL = `
      UPDATE sach
      SET TenSach = ?, TheLoai = ?, MaTacGia = ?, NamXuatBan = ?, SoLuong = ?, MaNXB = ?
      WHERE MaSach = ?`;
  
    db.query(updateSachSQL, [TenSach, TheLoai, MaTacGia, NamXuatBan, SoLuong, MaNXB, maSach], (err, result) => {
      if (err) {
...
router.delete('/:id', async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: 'Lỗi server khi xoá sách.' });
    }
  });
